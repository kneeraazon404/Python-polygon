...
class Para20Scanner(Scanner):
    ...
    async def run_per_ticker(self, session, ticker: Ticker):
        ...
        para_df = df.between_time('09:30', '20:00')  #TODO: Change to 1400
        ...
        para_df = para_df[(para_df["% from Prev Close"] > 0.13) & (para_df["VolumeSum"] > 250_000) & (para_df["Close"] >= 0.50)]

        prelims = {
            "Para (15min)": para_df[(para_df["Change 15 Minute"] > 0.18)],
            "Para (10min)": para_df[(para_df["Change 10 Minute"] > 0.16)],
            "Para (2min)": para_df[(para_df["Change 2 Minute"] > 0.14)],
        }
