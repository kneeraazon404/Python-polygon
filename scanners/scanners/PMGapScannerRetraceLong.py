...
class PMGapScannerRetraceLong(Scanner):
    ...
    async def run_per_ticker(self, session, ticker: Ticker):
        ...
        # target_symbol = "FULC"
        #
        # if ticker.symbol != target_symbol:
        #     return
        ...
        pm_df = df.between_time('09:29', '09:30')  #
        try:
            pm_df.insert(loc=1,
                     column="VolumeSum",
        ...
        joined_df["Gap %"] = round((joined_df["Close"] - joined_df["PrevClose"]) / joined_df["PrevClose"], 2)

        # pre_market_df = df.between_time('04:00', '09:31')
        # pre_market_df["PMRetrace %"] = round((pre_market_df["High"] - pre_market_df["Close"]) / pre_market_df["Close"], 2)
        #
        # joined_df = pd.merge(joined_df, pre_market_df[["Datetime", "PMRetrace %"]], how="left", on="Datetime")

        pre_market_df = df.between_time('04:00', '09:31')
        pre_market_df["PMRetrace %"] = round((pre_market_df["High"] - pre_market_df["Close"]) / pre_market_df["Close"],
                                             2)

        # Filter pre_market_df to get the row at 9:30
        pmretrace_9_30 = pre_market_df[pre_market_df["Datetime"].dt.time == pd.to_datetime('09:30:00').time()]

        # Get the PMRetrace % value at 9:30
        pmretrace_at_9_30 = pmretrace_9_30.iloc[0]["PMRetrace %"]

        # Add the PMRetrace % value to the joined_df DataFrame
        joined_df["PMRetrace % at 9:30"] = pmretrace_at_9_30
        ...
        results = {
            "30% Gap": joined_df[(joined_df["Gap %"] > 0.30) &
                                 (joined_df["VolumeSum"] > 100_000)
                                 # (joined_df["FirstBarPercentDiff"])
                                 # & (joined_df["PMRetrace %"] > 0.15)
                                 # (joined_df["PMRetrace %"] < 0.20)
                                 # (joined_df["Time"].dt.time >= pd.to_datetime('09:30:00').time()) &
                                 # (joined_df["Time"].dt.time <= pd.to_datetime('10:00:00').time())
            ],
            # "Filtered First Bar Results": filtered_first_bar_results,
        ...
        for resultkey in results:
            ...
            if not result.empty:
                ...
                result = result.loc[:, ["Date", "Datetime", "VolumeSum", "PrevClose", "Gap %", "PMRetrace % at 9:30"]]
