...
class ParabScannerScalp(Scanner):
    ...
    async def run_per_ticker(self, session, ticker: Ticker):
        ...
        para_df = para_df[(para_df["% from Prev Close"] > 0.05) & (para_df["VolumeSum"] > 250_000)
                          & (para_df["Close"] >= 3) & (para_df["Close"] <= 22.50) & (para_df["TodaysRange"] > 2)]

        prelims = {
            # "Para (15min)": para_df[(para_df["Change 15 Minute"] > 0.12)],
            # "Para (10min)": para_df[(para_df["Change 10 Minute"] > 0.10)],
            "Para (2min)": para_df[(para_df["Change 2 Minute"] > 0.01)],
        }
        ...
        if not results.empty:
            ...
            results = results[["Date", "Ticker", "Scan Time", "Scan Type", "Close", "% from Prev Close", "VolumeSum", "Change 2 Minute", "TodaysRange", "Count (Alerts)"]]  # TODO add "Count" & % Change from Open
