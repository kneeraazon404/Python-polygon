...
def get_market_cap(symbol):
    ...
    api_url = f"https://api.polygon.io/v3/reference/tickers/{symbol}?apiKey={api_key}"

    try:
        response = requests.get(api_url)
        data = response.json()

        # Assuming the market cap is available in the response, replace 'marketCap' with the actual key used by Polygon
...
class ParabScanner(Scanner):
    ...
    async def run_per_ticker(self, session, ticker: Ticker):
        ...
        para_df = df.between_time('09:30', '09:32')  #TODO: Change to 1400
        ...
        market_cap = get_market_cap(ticker.symbol)
        para_df["Market Cap"] = market_cap
        ...
        """
        This is the old one to use for testing regular paras
        """
        para_df = para_df[(para_df["VolumeSum"] > 150_000)
                          & (para_df["Close"] >= 1) & (para_df["Close"] <= 1000)
                          # & (para_df['Market Cap'] > 10000000000.00)
                          # & (para_df['Market Cap'] < 50000000000.00)
        ]

        """
        This is the old one I'm using to test Largecaps
        """
        # para_df = para_df[(para_df["VolumeSum"] > 250_000)
        #                   & (para_df["Close"] >= 5) & (para_df["Close"] <= 1000) & (para_df["Market Cap"] > 2000000000)]

        prelims = {
            "Para (15min)": para_df[(para_df["Change 15 Minute"] > 0.14)],
            "Para (10min)": para_df[(para_df["Change 10 Minute"] > 0.12)],
            "Para (2min)": para_df[(para_df["Change 2 Minute"] > 0.0125)],
        }
        ...
        if not results.empty:
            ...
            results = results[["Date", "Ticker", "Scan Time", "Scan Type", "Close", "% from Prev Close", "VolumeSum", "Change 2 Minute", "Change 10 Minute", "Change 15 Minute", "Count (Alerts)", "Market Cap"]]  # TODO add "Count" & % Change from Open

            # if "Market Cap" in results.columns:
            #     results = results[
            #         ["Date", "Ticker", "Scan Time", "Scan Type", "Close", "% from Prev Close", "VolumeSum",
            #          "Change 2 Minute", "Change 10 Minute", "Change 15 Minute", "Count (Alerts)", "Market Cap"]]
            # else:
            #     results = results[
            #         ["Date", "Ticker", "Scan Time", "Scan Type", "Close", "% from Prev Close", "VolumeSum",
            #          "Change 2 Minute", "Change 10 Minute", "Change 15 Minute", "Count (Alerts)"]]
